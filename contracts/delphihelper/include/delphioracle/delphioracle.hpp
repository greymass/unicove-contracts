...
public:
   ...
   */
   ...
   typedef eosio::multi_index<"stats"_n, stats, indexed_by<"count"_n, const_mem_fun<stats, uint64_t, &stats::by_count>>>
      statstable;
   ...
   typedef eosio::multi_index<"hashes"_n, hashes,
                              ...
                              indexed_by<"hash"_n, const_mem_fun<hashes, checksum256, &hashes::by_hash>>>
      hashestable;

   typedef eosio::multi_index<"voters"_n, voter_info,
                              indexed_by<"voter"_n, const_mem_fun<voter_info, uint64_t, &voter_info::primary_key>>>
      voters_table;
   ...
   typedef eosio::multi_index<"donations"_n, donations,
                              indexed_by<"donator"_n, const_mem_fun<donations, uint64_t, &donations::by_donator>>>
      donationstable;

   typedef eosio::multi_index<"users"_n, users, indexed_by<"score"_n, const_mem_fun<users, uint64_t, &users::by_score>>>
      userstable;

   typedef eosio::multi_index<"abusers"_n, abusers,
                              indexed_by<"votes"_n, const_mem_fun<abusers, uint64_t, &abusers::by_votes>>>
      abuserstable;
   ...
   [[eosio::on_notify("eosio.token::transfer")]] void transfer(uint64_t sender, uint64_t receiver)
   {
...
private:
   static uint128_t composite_key(const uint64_t& x, const uint64_t& y) { return (uint128_t{x} << 64) | y; }
   ...
   checksum256 get_multiparty_hash(const name owner, const std::string reveal)
   {
      ...
      while (itr != htable.rend()) {
         if (itr->owner != owner && itr->reveal != "") {
            ...
            std::string prev_multiparty =
               to_hex((char*)itr->multiparty.extract_as_byte_array().data(), sizeof(itr->multiparty));
   ...
   bool check_approver(const name owner)
   {
      ...
      if (gitr->approver_threshold == 0)
         return true;
      else if (itr != stats.end() && itr->count >= gitr->approver_threshold)
         return true;
      else
         return false;
   }
   ...
   void update_votes()
   {
      ...
      action act(permission_level{_self, "active"_n}, "eosio"_n, "voteproducer"_n, std::make_tuple(_self, ""_n, bps));
      act.send();
   }
   ...
   void update_datapoints(const name owner, const uint64_t value, pairstable::const_iterator pair_itr)
   {
      ...
      // set median
      t_idx.modify(oldest, _self, [&](auto& s) { s.median = median; });

      gtable.modify(gtable.begin(), _self, [&](auto& s) { s.total_datapoints_count++; });
   ...
   //   By proposing a bounty, the proposer pays upfront for all the RAM requirements of the pair (expensive enough to
   //   discourage spammy proposals)

   // Once the bounty has been created, anyone can contribute to the bounty by sending a transfer with the bounty name
   // in the memo

   // Custodians of the contract or the bounty proposer can cancel the bounty. This refunds RAM to the proposer, as well
   // as all donations made to the bounty
   ...
   // Any BP that has contributed a certain amount of datapoints (TBD) to the contract is automatically added as an
   // authorized account to approve a bounty
   ...
   void process_donation(name from, name scope, asset quantity)
   {
      ...
      users.modify(*uitr, _self, [&](auto& o) { o.score += quantity.amount; });
      ...
      auto count_index = cstore.get_index<"count"_n>(); // get list of oracles ranked by number of datapoints
                                                        // contributed for this scope (descending)
      ...
      for (uint64_t i = upperbound; i >= 1; i--) {
         ...
         if (scope == _self) {
            ...
            cstore.modify(*itr, _self, [&](auto& s) { s.balance += payout; });
            ...
            gstore.modify(*optr, _self, [&](auto& s) { s.balance += payout; });
         }
   ...
   void process_bounty(name from, name pair, asset quantity)
   {
      ...
      pairs.modify(*pitr, _self, [&](auto& s) { s.bounty_amount += quantity; });
   }
};