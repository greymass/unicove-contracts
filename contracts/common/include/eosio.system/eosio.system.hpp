...
static inline auto has_field(F flags, E field)
   -> std::enable_if_t<std::is_integral_v<F> && std::is_unsigned_v<F> && std::is_enum_v<E> &&
                          std::is_same_v<F, std::underlying_type_t<E>>,
                       bool>
{
...
static constexpr int64_t default_inflation_pay_factor =
   50000; // producers pay share = 10000 / 50000 = 20% of the inflation
static constexpr int64_t default_votepay_factor =
   40000; // per-block pay share = 10000 / 40000 = 25% of the producer pay
 ...
 * `setabi`, `setcode`), they are just declared in the contract so they will show in the contract's ABI and users will
 * be able to push those actions to the chain via the account holding the `eosio.system` contract, but the
...
typedef eosio::multi_index<"namebids"_n,
                           name_bid,
...
struct [[eosio::table, eosio::contract("eosio.system")]] producer_info
{
   ...
   eosio::block_signing_authority get_producer_authority() const
   {
      if (producer_authority.has_value()) {
         bool zero_threshold =
            std::visit([](auto&& auth) -> bool { return (auth.threshold == 0); }, *producer_authority);
   ...
   {
      ds << t.owner << t.total_votes << t.producer_key << t.is_active << t.url << t.unpaid_blocks << t.last_claim_time
         << t.location;
   ...
   {
      return ds >> t.owner >> t.total_votes >> t.producer_key >> t.is_active >> t.url >> t.unpaid_blocks >>
             t.last_claim_time >> t.location >> t.producer_authority;
   }
};
...
struct [[eosio::table("finalizers"), eosio::contract("eosio.system")]] finalizer_info
{
   ...
   uint64_t
      active_key_id; // finalizer's active finalizer key's id in finalizer_keys_table, for fast finding key information
   std::vector<char>
      active_key_binary; // finalizer's active finalizer key's binary format in Affine little endian non-montgomery g1
...
typedef eosio::multi_index<"producers"_n,
                           producer_info,
...
struct [[eosio::table, eosio::contract("eosio.system")]] gifted_ram
{
   ...
   {
      return giftee.value;
   } // unique as one giftee can only hold gifted ram from one gifter
};
...
// `rex_loan` structure underlying the `rex_cpu_loan_table` and `rex_net_loan_table`. A rex net/cpu loan table entry is
// defined by:
// - `version` defaulted to zero,
...
typedef eosio::multi_index<"cpuloan"_n,
                           rex_loan,
...
typedef eosio::multi_index<"netloan"_n,
                           rex_loan,
                           indexed_by<"byexpr"_n, const_mem_fun<rex_loan, uint64_t, &rex_loan::by_expr>>,
                           indexed_by<"byowner"_n, const_mem_fun<rex_loan, uint64_t, &rex_loan::by_owner>>>
   rex_net_loan_table;
...
typedef eosio::
   multi_index<"rexqueue"_n, rex_order, indexed_by<"bytime"_n, const_mem_fun<rex_order, uint64_t, &rex_order::by_time>>>
      rex_order_table;
...
struct powerup_config_resource
{
   std::optional<int64_t>
      current_weight_ratio; // Immediately set weight_ratio to this amount. 1x = 10^15. 0.01x = 10^13.
   ...
   std::optional<int64_t>
      target_weight_ratio; // Linearly shrink weight_ratio to this amount. 1x = 10^15. 0.01x = 10^13.
                           //    Do not specify to preserve the existing setting or use the default.
   ...
   EOSLIB_SERIALIZE(
      ...
      (current_weight_ratio)(target_weight_ratio)(assumed_stake_weight)(target_timestamp)(exponent)(decay_secs)(min_price)(max_price))
};
...
struct powerup_state_resource
{
   ...
   static constexpr uint32_t default_decay_secs =
      1 * seconds_per_day; // 1 day; if 100% of bandwidth resources are in a
...
public:
    ...
    * @param net_weight - fractionally proportionate net limit of available resources based on (weight /
    * total_weight_of_all_accounts),
    * @param cpu_weight - fractionally proportionate cpu limit of available resources based on (weight /
    * total_weight_of_all_accounts).
    */
   [[eosio::action]] void setalimits(const name& account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight);
    ...
    * total_weight_of_all_accounts).
    */
   [[eosio::action]] void setacctnet(const name& account, const std::optional<int64_t>& net_weight);
    ...
    * total_weight_of_all_accounts).
    */
   [[eosio::action]] void setacctcpu(const name& account, const std::optional<int64_t>& cpu_weight);
    ...
    */
   [[eosio::action]] void delegatebw(const name&  from,
                                     const name&  receiver,
    ...
    */
   [[eosio::action]] void setrex(const asset& balance);
    ...
    */
   [[eosio::action]] void cnclrexorder(const name& owner);
    ...
    */
   [[eosio::action]] void
   rentcpu(const name& from, const name& receiver, const asset& loan_payment, const asset& loan_fund);
    ...
    */
   [[eosio::action]] void
   rentnet(const name& from, const name& receiver, const asset& loan_payment, const asset& loan_fund);
    ...
    */
   [[eosio::action]] void fundcpuloan(const name& from, uint64_t loan_num, const asset& payment);
    ...
    * @param payment - tokens transferred from REX fund to loan fund.
    */
   [[eosio::action]] void fundnetloan(const name& from, uint64_t loan_num, const asset& payment);
    ...
    */
   [[eosio::action]] void defcpuloan(const name& from, uint64_t loan_num, const asset& amount);
    ...
    */
   [[eosio::action]] void defnetloan(const name& from, uint64_t loan_num, const asset& amount);
    ...
    */
   [[eosio::action]] void updaterex(const name& owner);
    ...
    */
   [[eosio::action]] void rexexec(const name& user, uint16_t max);

   /**
    * Consolidate action, consolidates REX maturity buckets into one bucket that can be sold after
    * {num_of_maturity_buckets} days starting from the end of the day.
    *
    * @param owner - REX owner account name.
    */
   [[eosio::action]] void consolidate(const name& owner);
    ...
    */
   [[eosio::action]] void closerex(const name& owner);
    ...
    */
   [[eosio::action]] void setrexmature(const std::optional<uint32_t> num_of_maturity_buckets,
    ...
    */
   [[eosio::action]] void undelegatebw(const name&  from,
                                       const name&  receiver,
    ...
    */
   [[eosio::action]] action_return_buyram buyram(const name& payer, const name& receiver, const asset& quant);
    ...
    */
   [[eosio::action]] action_return_buyram buyrambytes(const name& payer, const name& receiver, uint32_t bytes);

   /**
    * The buyramself action is designed to enhance the permission security by allowing an account to purchase RAM
    * exclusively for itself. This action prevents the potential risk associated with standard actions like buyram and
    * buyrambytes, which can transfer EOS tokens out of the account, acting as a proxy for eosio.token::transfer.
    *
    * @param account - the ram buyer and receiver,
    * @param quant - the quantity of tokens to buy ram with.
    */
   [[eosio::action]] action_return_buyram buyramself(const name& account, const asset& quant);
    ...
    */
   [[eosio::action]] void logbuyram(const name&  payer,
                                    const name&  receiver,
    ...
    */
   [[eosio::action]] action_return_sellram sellram(const name& account, int64_t bytes);
   ...
   giftram(const name from, const name to, int64_t bytes, const std::string& memo);
    ...
    */
   [[eosio::action]] void
   logsellram(const name& account, const asset& quantity, int64_t bytes, int64_t ram_bytes, const asset& fee);
    ...
    */
   [[eosio::action]] action_return_ramtransfer
   ramtransfer(const name& from, const name& to, int64_t bytes, const std::string& memo);
    ...
    * @param memo - the memo string to accompany the transaction.
    */
   [[eosio::action]] action_return_ramtransfer ramburn(const name& owner, int64_t bytes, const std::string& memo);
    ...
    * @param memo - the memo string to accompany the transaction.
    */
   [[eosio::action]] action_return_buyram buyramburn(const name& payer, const asset& quantity, const std::string& memo);
    ...
    * @param location - is the country code as defined in the ISO 3166,
    ...
    *
    * @param producer - account registering to be a producer candidate,
    * @param producer_authority - the weighted threshold multisig block signing authority of the block producer used to
    * sign blocks,
    * @param url - the url of the block producer, normally the url of the block producer presentation website,
    * @param location - is the country code as defined in the ISO 3166,
    * https://en.wikipedia.org/wiki/List_of_ISO_3166_country_codes
    ...
    * @pre Authority of producer to register
    */
   [[eosio::action]] void regproducer2(const name&                           producer,
    ...
    * @param proof_of_possession - a valid Proof of Possession signature to show the producer owns the private key of
    * the finalizer_key. The signature is in base64url format.
    *
    ...
    * @pre Authority of `finalizer_name` to register. `linkauth` may be used to allow a lower authrity to exectute this
    * action.
    */
   [[eosio::action]] void
   regfinkey(const name& finalizer_name, const std::string& finalizer_key, const std::string& proof_of_possession);
    ...
    * @param votepay_factor - Inverse of the fraction of the block producer rewards to be distributed proportional to
    * blocks produced. The remaining rewards will be distributed proportional to votes received. (eg. For 25% of block
    * producer rewards going towards block pay => votepay_factor = 40000 For 75% of block producer rewards going towards
    * block pay => votepay_factor = 13333).
    */
   [[eosio::action]] void setinflation(int64_t annual_rate, int64_t inflation_pay_factor, int64_t votepay_factor);
    ...
    * block pay => votepay_factor = 13333).
    */
   [[eosio::action]] void setpayfactor(int64_t inflation_pay_factor, int64_t votepay_factor);
   ...
   /**
    * Facilitates the removal of vested staked tokens from an account, ensuring that these tokens are reallocated to the
    * system's pool.
    *
    ...
    * action is invoked.
    */
   [[eosio::action]] void cfgpowerup(powerup_config& args);
    ...
    */
   [[eosio::action]] void powerupexec(const name& user, uint16_t max);
    ...
    */
   [[eosio::action]] void powerup(const name&  payer,
                                  const name&  receiver,
                                  uint32_t     days,
                                  int64_t      net_frac,
                                  int64_t      cpu_frac,
    ...
    */
   [[eosio::action]] void
   limitauthchg(const name& account, const std::vector<name>& allow_perms, const std::vector<name>& disallow_perms);
...
private:
   ...
   asset             update_rex_account(const name&  owner,
   ...
   void    consolidate_rex_balance(const rex_balance_table::const_iterator& bitr, const asset& rex_in_sell_order);
   ...
   void changebw(
      name from, const name& receiver, const asset& stake_net_quantity, const asset& stake_cpu_quantity, bool transfer);
   ...
   void    update_stake_delegated(const name  from,
                                  const name  receiver,
                                  const asset stake_net_delta,
                                  const asset stake_cpu_delta);
   void    update_user_resources(const name  from,
                                 const name  receiver,
                                 const asset stake_net_delta,
                                 const asset stake_cpu_delta);

   // defined in voting.cpp
   void   register_producer(const name&                           producer,
                            const eosio::block_signing_authority& producer_authority,
                            const std::string&                    url,
                            uint16_t                              location);
   ...
   void adjust_resources(name    payer,
                         name    account,
                         symbol  core_symbol,
                         int64_t net_delta,
                         int64_t cpu_delta,
                         bool    must_not_be_managed = false);
   void process_powerup_queue(time_point_sec       now,
                              symbol               core_symbol,
                              powerup_state&       state,
   ...
   void add_to_blockinfo_table(const eosio::checksum256&    previous_block_id,
